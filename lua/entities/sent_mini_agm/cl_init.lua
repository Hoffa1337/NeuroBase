...
function ENT:Draw()
	...
	-- if (self.cooltime < CurTime()) then
		local smoke = emitter:Add("effects/smoke_a", self:GetPos() + self:GetForward()*-15)
		...
		local fire = emitter:Add("effects/smoke_a", self:GetPos() + self:GetForward()*-15)
		...
		local fire = emitter:Add("effects/yellowflare", self:GetPos() + self:GetForward()*-15)
		fire:SetVelocity(self:GetForward()*-10)
