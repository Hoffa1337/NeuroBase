...
function SWEP:Think()
	...
	if( !self.Target ) then 
		...
		if ( trace.Hit && IsValid( trace.Entity ) ) then
			...
			if( type( te ) == "CSENT_vehicle" || te:IsVehicle() || te:IsNPC() || te:IsPlayer() || te.HealthVal) then
...
function SWEP:LaunchRocket()
	...
	local rocket = "sent_np_stinger_rocket"
	
	if( NEUROPLANES_CINEMATIC_ROCKET ) then
				
		rocket = "sent_a2l_missile"
		
	end
	
	self.Missile = ents.Create("sent_np_stinger_rocket")
	...
	if( NEUROPLANES_CINEMATIC_ROCKET ) then
		...
		self.Missile.Pointer = self.Owner
...
function SWEP:DrawHUD()
	...
	local tr, trace = {},{}
	tr.start =  self.Owner:GetShootPos() + self.Owner:GetForward() * 100 + self.Owner:GetUp() * 16
	tr.endpos = self.Owner:GetAimVector() * 999999999
	tr.filter = { self, self.Owner }
	tr.mask = MASK_SOLID
	trace = util.TraceEntity( tr, self.Owner )
	...
	local target = self:GetNetworkedEntity("Target", self )
